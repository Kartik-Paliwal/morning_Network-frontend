...
const getNewsItemsHelper = async () => {
	try {
		const response = await axios.get('https://morning-network-backend.onrender.com/api/news');
		return {
			news: response.data
		};
	} catch (error) {
...
const getNewsItemByIdHelper = async (id) => {
	try {
		const response = await axios.get(`https://morning-network-backend.onrender.com/api/news/${id}`);
		return {
